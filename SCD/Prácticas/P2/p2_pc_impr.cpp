...
// DNI: 77432800B
// Compilado con: g++ -std=c++11 -pthread -o p2_pc_impr p2_pc_impr.cpp scd.cpp
...
class ProdConsSU1 : public HoareMonitor
{
 ...
 int                        // variables permanentes
   ...
   primera_ocupada ,        //   indice d celda de la próxima extracción
   num_ocupadas ,
   num_mult ,
   num_mult_total ;
...
{
   if ( num_mult_total < num_items / 5) {
      if (num_mult == 0)
         impresora.wait();
      cout << "Se han introducido " << num_mult << " multiplo desde la ultima llamada" << endl;
      num_mult=0;
      return true;
   }
   else return false;
...
void funcion_hebra_impresora( MRef<ProdConsSU1> monitor )
{
   while(monitor->metodo_impresora()) {
      this_thread::sleep_for( chrono::milliseconds( aleatorio<min_ms*2,max_ms*2>() ));
   }
}
...
int main()
{
   ...
   thread hebra_prod( funcion_hebra_productora, monitor ),
          ...
          hebra_impr( funcion_hebra_impresora, monitor);
