...
// DNI: 77432800B
// Compilado con: mpicxx -std=c++11 -o prodcons2-mu prodcons2-mu.cpp
// -----------------------------------------------------------------------------
//
...
const int
   ...
   num_items             = 20,
   num_por_prod          = num_items / num_productores ,
   num_por_cons          = num_items / num_consumidores,
   tam_vector            = 10;
...
int producir(int id)
{
   ...
   contador++ ;
   int valor = contador + (id*num_por_prod);
...
{
   for ( unsigned int i= 0 ; i < num_por_prod ; i++ )
   {
      // producir valor
...
void funcion_consumidor( int id)
{
   ...
   for( unsigned int i=0 ; i < num_por_cons; i++ )
   {
...
void funcion_buffer()
{
   int        buffer[tam_vector],      // buffer con celdas ocupadas y vacías
              ...
              etiq_aceptable ;    // etiqueta de mensaje aceptable
   ...
   for( unsigned int i=0 ; i < num_items*2 ; i++ )
   {
      ...
      switch( estado.MPI_TAG ) // leer emisor del mensaje en metadatos
      {
         case etiq_productores: // si ha sido el productor: insertar en buffer
            ...
            num_celdas_ocupadas++ ;
            cout << "Buffer ha recibido valor " << valor << " de productor " << estado.MPI_SOURCE << endl ;
            break;
         ...
         case etiq_consumidores: // si ha sido el consumidor: extraer y enviarle
            ...
            num_celdas_ocupadas-- ;
            cout << "Buffer va a enviar valor " << valor << " a consumidor " << estado.MPI_SOURCE - num_productores - 1 << endl ;
